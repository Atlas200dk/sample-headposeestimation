...
function main()
{
    ...
    presenter_port=`grep presenter_server_port ${script_path}/../presenterserver/Head_pose/config/config.conf | awk -F '=' '{print $2}' | sed 's/[^0-9]//g'`
    if [ $? -ne 0 ];then
        echo "ERROR: get presenter server port failed, please check ${script_path}/../presenterserver/Head_pose/config/config.conf."
        return 1
    fi
